...
export default function ChallengeDreamsScreen() {
  ...
  const fetchDreams = useCallback(async () => {
    if (!token) return;
    try {
      const response = await fetch(new URL('/api/dreams?type=challenge', getApiUrl()).toString(), {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (response.ok) {
        const data = await response.json();
        setDreams(data.filter((d: Dream) => d.type === "challenge"));
      }
    } catch (error) {
  ...
  const handleCreateDream = () => {
    ...
    navigation.navigate("CreateDream", { type: "challenge" });
  };
      ...
      >
          ...
          <Animated.View entering={FadeInDown.springify()}>
            <Card style={styles.emptyCard}>
              <View style={styles.emptyIcon}>
                <Feather name="award" size={40} color="#EAB308" />
              </View>
              <ThemedText type="body" style={styles.emptyTitle}>No Challenges Yet</ThemedText>
              <ThemedText type="small" style={styles.emptyDescription}>
                Compete with others by creating a challenge dream
              </ThemedText>
            </Card>
          </Animated.View>
        ) : (
        ...
        <Animated.View entering={FadeInDown.delay(300).springify()}>
          <Button
            ...
            testID="button-create-challenge-dream"
          >
