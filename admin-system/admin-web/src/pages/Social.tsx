        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out h-full flex flex-col">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-400 text-sm">Review global news feed activity and user engagement.</p>
                </div>
                ...
                <div className="overflow-x-auto flex-1 custom-scrollbar">
                    <table className="w-full text-sm text-left text-slate-300">
                        <thead className="text-xs text-slate-400 uppercase bg-slate-900/80 sticky top-0 z-10 border-b border-slate-700/50 backdrop-blur-md">
                            <tr>
                                <th scope="col" className="px-6 py-4 font-medium tracking-wider">Author</th>
                                <th scope="col" className="px-6 py-4 font-medium tracking-wider">Content Snippet</th>
                                <th scope="col" className="px-6 py-4 font-medium tracking-wider">Related Dream</th>
                                <th scope="col" className="px-6 py-4 font-medium tracking-wider">Engagement</th>
                                ...
                                filteredPosts.map((post) => (
                                    <tr key={post.id} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            {post.dream_title ? (
                                                <span className="text-xs text-indigo-300 bg-indigo-500/10 px-2 py-1 rounded border border-indigo-500/20 truncate max-w-[150px] inline-block">
                                                    {post.dream_title}
                                                </span>
                                            ) : (
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-4 text-xs text-slate-400">
                                                <span className="flex items-center gap-1"><Heart size={14} className="text-rose-400/70" /> {post.likes}</span>
                                                <span className="flex items-center gap-1"><MessageSquare size={14} className="text-sky-400/70" /> {post.comments}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <button className="text-red-400 hover:text-red-300 hover:bg-red-400/10 px-3 py-1.5 rounded transition-colors text-xs font-medium border border-transparent hover:border-red-500/30">
                                                Remove
                                            </button>
                                        </td>
                                    </tr>
